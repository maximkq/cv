...
I hope to start developing games as a team in the future :EMOJICODE:.
...
iti = mp.begin();
							...
							for (int i = 0; iti != mp.end(); i++, iti++) {
								for (int j = mp.size() - 1; j > i; ati--, j--) {
									...
									if (second_noti.GetYear() > noti.GetYear() || (second_noti.GetYear() == noti.GetYear() && (second_noti.GetMonth() > noti.GetMonth()
										|| (second_noti.GetMonth() == noti.GetMonth() && second_noti.GetDay() > noti.GetDay())))) {
										swap(mp[j], mp[j - 1]);
										ati++;
									}
								}
